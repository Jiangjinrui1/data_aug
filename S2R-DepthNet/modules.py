...
class GaussianNoiseLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        noise = (torch.randn_like(x, device=x.device) - 0.5) / 10.0
        return x+noise
...
class Depth_Net(nn.Module):
    ...
    def forward(self, input):
        conv1 = self.pool(self.conv1(input))          # 3/64 1/2 torch.Size([1, 64, 200, 300])
        conv2 = self.pool(self.conv2.forward(conv1))  # 64/128 1/4 torch.Size([1, 128, 100, 150])
        conv3 = self.pool(self.conv3.forward(conv2))  # 128/256 1/8 torch.Size([1, 256, 50, 75])
        ...
        scale = 1.0
        result= []
        # center_out = torch.nn.functional.interpolate(center_out, size=(50, 75), mode='bilinear', align_corners=True)
