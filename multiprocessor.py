...
def person_detect(img):
    ...
    if len(person_detections) > 0:
        ...
        if boxes is None:
            logging.info("No face detected, returning person bounding box center.")
            # 获取第一个 person 检测框的中心点
            ...
            print(f"Person center coordinates: ({center_x}, {center_y})")
...
def generate_face_edit_prompt(client, face_description, model="gpt-4"):
    """
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "Generate a concise, descriptive sentence focused on facial features without any extra instructions."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=25  # 限制生成的文本长度，以确保简洁
    )
...
def pipeline_process_images_from_folder(
    ...
    """并行处理文件夹中的图像并根据 caption 文件生成 prompt"""
    ...
    with open(caption_file, 'r') as orig_f, open(cap_modified_file, 'r') as counter_f:
        ...
        for orig_line, counter_line in zip(orig_f, counter_f):
            original_data = ast.literal_eval(orig_line)
            ...
            if os.path.exists(os.path.join(output_dir, img_folder_name)) or img_id in processed_img:
                logging.info(f"Skipping already processed image: {img_id}")
                continue
            ...
            tasks.append((img_path, counterfactual_title, original_title))

    # 使用多线程并行处理任务
    def process_task(task):
        img_path, counterfactual_title, original_title = task
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                future.result()
                logging.info(f"Successfully processed image: {task[0]}")
            except Exception as e:
                logging.error(f"Error processing image {task[0]}: {e}")
...
def process_single_image(
    ...
    if face_coords:
        ...
        text_prompt = generate_summarized_counterfactual_prompt(client, original_prompt, text_prompt)
        logging.info(f"Processing image: {input_img} with prompt: {text_prompt}")
    ...
    # 选择得分最高的图像并保存
    best_image = select_best_image_pair(image_pairs, original_prompt, text_prompt)
    if best_image:
